// The scene's frame-budget sampler, pure so the step-down it drives can be fed a sequence of
...
//   a stutter. Frames at or over SLOW are kept OUT of this mean: the first frames of the
//   entrance compile the physical material's shaders (~150 ms each) and would have tripped
//   a fast GPU on its first window.
// - SLOW COUNT: frames at or over SLOW, counted on their own. Catches the GPU whose EVERY
//   entrance frame is over 100 ms — the class the step-down exists for — which the mean
//   trigger can never see because it never accumulates a sample. A compile blip is two or
//   three such frames; SLOW_LIMIT is set where only a sustained ≤ 10 fps device arrives.
//
...
// lowers DPR, then switches the neighbour occlusion off) and there are at most MAX_STEPS of
// them; nothing here steps back up, so the picture cannot flap.
...
export const PERF = {
  ...
  SLOW: 0.1,
  /** slow frames in one entrance that degrade — a compile blip is 2–3, a 10 fps entrance is 20+ */
  SLOW_LIMIT: 12,
  /** seconds; at or over this the delta is an idle or hidden-tab gap, not a frame */
  GAP: 1,
  /** DPR 1, then no neighbour occlusion */
  MAX_STEPS: 2,
} as const;
